...
impl Chain {
    ...
    pub fn validate(
        ...
        // One ballot per key image. This is the double-vote check: the key image is a
        // deterministic function of the signer's private key and the election.
        let mut new_blocks_key_images = HashSet::new();
        for i in 1..new_blocks.len() {
            ...
            if !Block::validate(&new_blocks[i], election) {
                return Err(format!("block {} failed validation", i));
            }
            if new_blocks_key_images.insert(new_blocks[i].key_image.clone()) {
                prev_block_hash = new_blocks[i].hash.clone();
            } else {
                return Err(format!(
                    "block {} has a duplicate key image within the new chain (double vote)",
                    i
                ));
            }
        }
    ...
    pub fn validate_addition(
        seen_key_images: &HashSet<Vec<u8>>,
        current_chain: &[Block],
        new_block: &Block,
        election: &ElectionManifest,
    ) -> ValidateChainRes {
        // If this key image already voted, ignore.
        if seen_key_images.contains(&new_block.key_image) {
        ...
        if current_chain.is_empty() {
            ...
            if new_block.idx == 0 {
                if Block::validate(new_block, election) {
