...
struct Args {
    ...
    #[arg(
        ...
        help = "Path to the election manifest exported from the admin panel (see ELECTION_MANIFEST.md)"
    )]
    election: String,
}
...
async fn main() {
    ...
    // Every node imports its own manifest from disk — nothing about the electorate arrives over
    // the network. See ELECTION_MANIFEST.md. Failing here costs a restart; failing to notice a
    // bad electorate until ballots are cast costs the election.
    let manifest = match ElectionManifest::load(Path::new(&args.election)) {
        Ok(manifest) => manifest,
        Err(reason) => {
            eprintln!("Cannot start: {}", reason);
            std::process::exit(1);
        }
    };
    println!(
        "Election: {} ({})\n  candidates: {}\n  anonymity groups: {} covering {} public keys",
        manifest.title,
        manifest.election_id,
        manifest.candidate_count(),
        manifest.ring_count(),
        manifest.voter_count(),
    );
